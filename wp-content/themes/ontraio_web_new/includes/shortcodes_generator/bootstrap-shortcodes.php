...
class BoostrapShortcodes {
  ...
  function bootstrap_shortcodes_scripts()  { 
    ...
    wp_enqueue_script( 'bootstrap-shortcodes-tooltip',  'includes/js/bootstrap-shortcodes-tooltip.js', array( 'jquery' ), false, true );
    
    // Bootstrap popover js
    wp_enqueue_script( 'bootstrap-shortcodes-popover', 'includes/js/bootstrap-shortcodes-popover.js', array( 'jquery' ), false, true );
  ...
  */
  ...
  function bs_tabs( $atts, $content = null ) {
    ...
    return sprintf( 
      '<div class="global_tab_wrap"><ul class="%s" id="%s"%s>%s</ul><div class="global_tab"><div class="global_tab_inner">
                        	<div class="inner"><div class="%s">%s</div></div></div></div></div>',
      esc_attr( $ul_class ),
      esc_attr( $id ),
      ( $data_props ) ? ' ' . $data_props : '',
      ( $tabs )  ? implode( $tabs ) : '',
  ...
  function bs_tab( $atts, $content = null ) {
    ...
    return sprintf( 
      '<div id="%s" class="%s global_inner_tab"%s>%s</div>',
      esc_attr( $id ),
      esc_attr( $class ),
      ( $data_props ) ? ' ' . $data_props : '',
      do_shortcode( $content )
  ...
  function bs_collapsibles( $atts, $content = null ) {
    ...
    return sprintf( 
      '<div class="askari_accordian"><div class="%s" id="%s"%s>%s</div></div>',
      esc_attr( $class ),
      esc_attr( $id ),
      ( $data_props ) ? ' ' . $data_props : '',
      do_shortcode( $content )
    ...
    return sprintf( 
      '<div class="%1$s"%2$s>
        ...
        <div id="%4$s" class="%6$s">
          <div class="panel-body"><Div class="acc_tab_wrapper"><div class="inner">%7$s</div>
        </div>
      </div>
	  </div>
      </div>',
